...
std::vector<double> PageRankSolverMPI::solve(const COOMatrix& M, const int max_iter, const double tol) {
    ...
    // Filter COO matrix entries to only local rows
    ...
    for (size_t k = 0; k < M.values.size(); ++k) {
        ...
        if (global_i >= start_row && global_i < start_row + local_rows) {
            local_i.push_back(global_i - start_row); // local row index
    ...
    std::vector<double> u(N, 1.0 / N);                // global vector
    std::vector<double> local_result(local_rows, 0.0);  // local part
    std::vector<double> gathered_result(N, 0.0);      // full gathered result
    ...
    for (int iter = 0; iter < max_iter; ++iter) {
        ...
        // Gather all local_results into full gathered_result
